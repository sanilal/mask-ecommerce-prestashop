...
class CustomHtmlClass extends ObjectModel
{
	...
	public	function __construct($id_tab = null, $id_lang = null, $id_shop = null, Context $context = null)
	{
		...
		Shop::addTableAssociation('labcustomhtml', array('type' => 'shop'));
		Shop::addTableAssociation('labcustomhtml_lang', array('type' => 'fk_shop'));
		if ($this->id)
		{
	...
	public function getFieldShop($field)
	{
		...
		$sql = 'SELECT bs.'.$field.' FROM '._DB_PREFIX_.'labcustomhtml b
		LEFT JOIN '._DB_PREFIX_.'labcustomhtml_shop bs ON (b.id_labcustomhtml = bs.id_labcustomhtml)
		WHERE b.id_labcustomhtml = '.$this->id.' AND bs.id_shop = '.$id_shop.'';
