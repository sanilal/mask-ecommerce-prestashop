...
include_once('labslideshow.php');

$context = Context::getContext();
$home_slider = new labslideshow();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
	{
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'labslideshow_slides` SET `position` = '.(int)$position.'
			WHERE `id_labslideshow_slides` = '.(int)$id_slide
		);
	}
