...
class labslidesho extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'labslideshow_slides',
		'primary' => 'id_labslideshow_slides',
		'multilang' => true,
		'fields' => array(
			...
			'margin' =>			array('type' => self::TYPE_STRING, 'validate' => 'isCleanHtml', 'size' => 255),
			'style' =>			array('type' => self::TYPE_STRING, 'validate' => 'isCleanHtml', 'size' => 255),
		)
	);
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'labslideshow` (`id_shop`, `id_labslideshow_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'labslideshow`
			WHERE `id_labslideshow_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_labslideshow_slides` = hs.`id_labslideshow_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_labslideshow_slides` as id_slide
			FROM `'._DB_PREFIX_.'labslideshow_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'labslideshow` hs ON (hss.`id_labslideshow_slides` = hs.`id_labslideshow_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new labslidesho($row['id_slide']);
