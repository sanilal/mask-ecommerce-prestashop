...
class LabMegamenuClass extends ObjectModel
{
	public $type_link;
	public $dropdown;
	public $type_icon;
	...
	public	function __construct($id_labmegamenu = null, $id_lang = null, $id_shop = null, Context $context = null)
	{
		parent::__construct($id_labmegamenu, $id_lang, $id_shop);
		Shop::addTableAssociation('labmegamenu', array('type' => 'shop'));
		Shop::addTableAssociation('labmegamenu_lang', array('type' => 'fk_shop'));
		if ($this->id)
	...
	public function getFieldShop($field)
	{
		...
		$sql = 'SELECT wms.'.$field.' FROM '._DB_PREFIX_.'labmegamenu wm
		LEFT JOIN '._DB_PREFIX_.'labmegamenu_shop wms ON (wm.id_labmegamenu = wms.id_labmegamenu)
		WHERE wm.id_labmegamenu = '.$this->id.' AND wms.id_shop = '.$id_shop.'';
	...
	public function delete()
	{
		...
		if (preg_match('/sample/', $icon) === 0)
			if ($icon && file_exists(_PS_MODULE_DIR_.'labmegamenu/views/img/icons/'.$icon))
				$res &= @unlink(_PS_MODULE_DIR_.'labmegamenu/views/img/icons/'.$icon);
	...
	public function getColumByRow($id_row)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mc.*
			FROM '._DB_PREFIX_.'labmegamenu_column mc
	...
	public function deleteMenuItem($id_col)
	{
		...
		$menu_items = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mi.*
			FROM '._DB_PREFIX_.'labmegamenu_item mi
		...
		if (isset ($menu_items) && count($menu_items) > 0)
		{
			foreach ($menu_items as $menu_item)
			{
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'labmegamenu_item_lang`
					WHERE `id_item` = '.$menu_item['id_item']
				);
				
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'labmegamenu_item_shop`
					WHERE `id_item` = '.$menu_item['id_item']
				);
				
				$res &= Db::getInstance()->execute('
					DELETE FROM `'._DB_PREFIX_.'labmegamenu_item`
					WHERE `id_item` = '.$menu_item['id_item']
				);
			}
		}
	...
	public function deleteColumnItem($id_col)
	{
		$res = true;
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'labmegamenu_column_shop`
			WHERE `id_column` = '.$id_col
		);
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'labmegamenu_column`
				WHERE `id_column` = '.$id_col
			);
		return $res;
	}
	...
	public function deleteRowItem($id_row)
	{
		$res = true;
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'labmegamenu_row_shop`
				WHERE `id_row` = '.$id_row
			);
		
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'labmegamenu_row`
				WHERE `id_row` = '.$id_row
			);
		return $res;
	}
	...
	public function getRowByMenu($id_menu)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mr.*
			FROM '._DB_PREFIX_.'labmegamenu_row mr
			WHERE mr.`id_labmegamenu` = '.$id_menu);
	}
	...
	public function getMenus()
	{
		...
		$kq = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT lab.*, ll.*
			FROM '._DB_PREFIX_.'labmegamenu_shop lab
			LEFT JOIN `'._DB_PREFIX_.'labmegamenu_lang` ll ON (ll.`id_labmegamenu` = lab.`id_labmegamenu` AND lab.`id_shop` = ll.`id_shop`)
			WHERE lab.active = 1 AND ll.id_shop = '.$id_shop.' AND ll.id_lang='.$id_lang.' ORDER BY lab.position ASC, lab.id_labmegamenu ASC');
		return $kq;
	}
	...
	public function getPositionByMenu($id_menu)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			SELECT ms.`position`
			FROM '._DB_PREFIX_.'labmegamenu_shop ms
			WHERE ms.`id_labmegamenu` = '.$id_menu.' AND ms.`id_shop` = '.$id_shop);
	}
}