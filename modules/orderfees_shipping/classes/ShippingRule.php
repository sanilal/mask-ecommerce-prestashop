...
class ShippingRule extends ObjectModel
{
                ...
                foreach ($zipcode_rules as $zipcode_rule) {
                    ...
                    if ($operator == '!=') {
                        $values = array_map(function ($v) {
                            return trim(Tools::strtolower($v));
                        }, $values);
    ...
    public function value($use_tax)
    {
        $cart = $this->cart;
        $products = $cart->getProducts();
        $value = 0;
        
        $cache_id = sprintf('orderfees_shipping_%d_%d_%d', (int)$this->id, (int)$use_tax, (int)$cart->id);
        ...
        if ($this->type & self::IS_PERCENT) {
            ...
            if ($this->percent && $this->product == 0) {
                $value += $cart->getOrderTotal($use_tax, Cart::BOTH_WITHOUT_SHIPPING) * $this->percent / 100;
            }
            ...
            if ($this->percent && $this->product > 0) {
                foreach ($products as $product) {
                    if ($product['id_product'] == $this->product) {
                        $value += ($use_tax ? $product['total_wt'] : $product['total']) * $this->percent / 100;
                    }
                }
            }
                ...
                if (is_array($selected_products)) {
                    foreach ($cart->getProducts() as $product) {
                        if (in_array($product['id_product'].'-'.$product['id_product_attribute'], $selected_products)
                            ...
                            $selected_products_total += ($use_tax ? $product['total_wt'] : $product['total']);
                        }
                    }
                }
        ...
        if (($this->type & self::IS_FORMULA) && $this->formula) {
            $data = array(
                'total' => $cart->getOrderTotal($use_tax, Cart::BOTH_WITHOUT_SHIPPING)
            );
            ...
            if (is_array($selected_products)) {
                ...
                foreach ($cart->getProducts() as $product) {
                    ...
                    foreach ($dimensions_available as $dim) {
                        ...
                        if (isset($product[$dim])) {
                            $data[$dim] += ($product[$dim] * $product['quantity']);
                        }
                    }
                    ...
                    $data['volume'] += ($volume * $product['quantity']);
                }
    ...
    public function restrictionsProducts($return_products = false)
    {
        ...
        if (is_array($products = $cart->getProducts())) {
            foreach ($products as $product) {
                $selected_products[] = (int)$product['id_product'].'-'.(int)$product['id_product_attribute'];
            }
        }
            ...
            foreach ($product_rules as $product_rule) {
                switch ($product_rule['type']) {
                    case 'features':
                        ...
                        AND cp.`id_product` IN ('.implode(',', array_map('intval', $selected_products)).')');
                        ...
                        $selected_products = self::arrayUintersect($selected_products, $matching_products_list);
                        break;
                    case 'attributes':
                        ...
                        foreach ($cart_attributes as $ca) {
                            if (in_array($ca['id_attribute'], $product_rule['values'])) {
                    ...
                    case 'categories':
                        ...
                        foreach ($cart_categories as $cc) {
                            if (in_array($cc['id_category'], $product_rule['values'])
                                ...
                                $matching_products_list[] = $cc['id_product'].'-'.$cc['id_product_attribute'];
                            }
                        }
                        if ($count_matching_products < $product_rule_group['quantity']) {
                            if ($countRulesProduct === 1) {
